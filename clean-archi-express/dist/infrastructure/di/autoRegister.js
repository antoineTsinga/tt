...
import { container, container as rootContainer, } from "tsyringe";
import { performance } from "node:perf_hooks";
import { snapshotContainer, } from "./containerIntrospection.js";
        ...
        if (infosAfter.length > infosBefore.length) {
            added.push({
                ...
                infos: infosAfter.slice(infosBefore.length).map((i) => ({
                    ...
                    element: getClassName(i.provider),
                })),
            });
        }
    }
...
function getClassName(element) {
    ...
    // useFactory: on PEUT essayer d'exécuter la factory en DEV
    if (element.useFactory) {
        // ⚠️ on ne fait ça qu'en dev
    ...
    if (element.useToken) {
        ...
        if (typeof t === "function" && t.name)
            return `token(class ${t.name})`;
        if (typeof t === "symbol")
            return `token(${t.toString()})`;
        return "token";
    }
    return undefined;
}
function introspectFactoryReturnTypeName(factory, container) {
 ...
 */
export async function autoRegister({ roots = [rootFromDir], patterns = [
    "**/*.registrar.[tj]s",
    "**/*.registry.[tj]s",
    "**/*.@(controller|repository|usecases|service).[tj]s",
], strict = false, container = rootContainer, } = {}) {
    ...
    console.log(`  Racine : ${chalk.green(roots.join(", "))}   (${files.length} fichiers scannés, ${durationMs.toFixed(1)} ms)`);
    if (!files.length) {
    ...
    if (!added.length) {
        ...
        added.forEach(({ token, infos }) => {
            ...
            console.log(`    ${chalk.green("✔")} ${chalk.white(tokenLabel)}  →  ${chalk.cyan(kinds.join(", "))} { ${infos.map((i) => i.element).join(", ")} }`);
        });
    }
    console.log("");
    return {
