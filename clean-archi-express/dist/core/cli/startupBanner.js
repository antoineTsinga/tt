...
export function printStartupCli(opts) {
    ...
    const formatRow = (key, value) => chalk.green("│") +
    ...
    if (modules.length > 0) {
        ...
        modules.forEach((m) => {
            ...
            moduleLines.push(`${chalk.gray("[App]")} ${statusIcon} ${chalk.white(pad(m.name, 24))} ${time}`);
        });
    }
    // 4) Diagnostic ligne type Nest
    const diagLine = chalk.gray("[Diag]") +
        " " +
